class VinylDesk {
    ...
    async handleLogin(email, password) {
        try {
            ...
            if (response.ok) {
                ...
                this.updateUserUI();
                this.showNotification(`🎉 Welcome back, ${this.user.username}!`, 'success', 3000);
            } else {
                this.showNotification(data.message || 'Login failed', 'error', 3000);
            }
        } catch (error) {
            this.showNotification('Connection error', 'error', 3000);
        }
    }
    ...
    async handleSignup(username, email, password) {
        try {
            ...
            if (response.ok) {
                ...
                document.getElementById('authModal').style.display = 'none';
                this.updateUserUI();
                this.showNotification(`🎵 Welcome, ${this.user.username}!`, 'success', 3000);
            } else {
                this.showNotification(data.message || 'Signup failed', 'error', 3000);
            }
        } catch (error) {
            this.showNotification('Connection error', 'error', 3000);
        }
    ...
    updateUserUI() {
        ...
        if (this.user) {
            ...
            if (menuUsername) menuUsername.textContent = this.user.username;
            if (menuAvatar) menuAvatar.textContent = this.user.avatar || '🎵';
            
            // Update stats
            if (statHighScore) statHighScore.textContent = 
                this.user.gameStats?.beatMatcher?.highScore || 0;
            if (statFish) statFish.textContent = 
                this.user.gameStats?.fishCollection?.fishCaught || 0;
            if (statTracks) statTracks.textContent = 
                this.user.stats?.totalTracksPlayed || 0;
        } else {
    ...
    showUserMenu() {
        ...
        if (menu) {
            menu.style.display = this.userMenuVisible ? 'block' : 'none';
            
            if (this.userMenuVisible && !this.user) {
                // Show login prompt in menu
                menu.innerHTML = `
                    <div class="user-menu-header">
                        <span>Guest</span>
                        <span>🎵</span>
                    </div>
                    <div class="user-menu-item" onclick="window.desk.showAuthModal()">
    ...
    logout() {
        ...
        this.updateUserUI();
        this.showNotification('👋 Logged out', 'info', 2000);
        document.getElementById('userMenu').style.display = 'none';
    }
    
    async saveGameStats(game, score, extras = {}) {
        if (!this.user) return;
        
        try {
            await fetch(`${this.apiUrl}/users/gamestats`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${this.token}`
                },
                body: JSON.stringify({ game, score, ...extras })
            });
            this.fetchUserProfile();
        } catch (error) {
    ...
    async saveRecordToCollection(record) {
        if (!this.user) {
            this.showNotification('🔐 Login to save records!', 'warning', 3000);
            return;
        }
        
        try {
            await fetch(`${this.apiUrl}/records`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${this.token}`
            ...
            this.showNotification('💿 Record saved!', 'success', 2000);
        } catch (error) {
    ...
    createFish() {
        ...
        fish.addEventListener('click', () => {
            ...
            // Save fish to backend if logged in
            if (this.user) {
                this.saveGameStats('fishCollection', 0, { fishCaught: 1 });
            }
    ...
    stopBeatMatcher() {
        ...
        if (this.user) {
            this.saveGameStats('beatMatcher', this.beatMatcherScore, {
                perfectHits: this.beatMatcherPerfectHits
            });
        }
    }
    ...
    stopRhythmGame() {
        ...
        // Save score to backend if logged in
        if (this.user) {
            this.saveGameStats('rhythmGame', this.rhythmGameScore, {
                combo: this.rhythmGameMaxCombo
            });
        }
    }
...
function switchAuthTab(tab) {
    if (window.desk) window.desk.switchAuthTab(tab);
}
...
function handleLogin(event) {
    ...
    if (window.desk) window.desk.handleLogin(email, password);
}
...
function handleSignup(event) {
    ...
    if (window.desk) window.desk.handleSignup(username, email, password);
}

function showUserMenu() {
    if (window.desk) window.desk.showUserMenu();
}

function logout() {
    if (window.desk) window.desk.logout();
}

function viewCollection() {
    if (window.desk) window.desk.showNotification('📀 Collection view coming soon!', 'info', 2000);
}

function viewPlaylists() {
    if (window.desk) window.desk.showNotification('📋 Playlists coming soon!', 'info', 2000);
}

function viewAchievements() {
    if (window.desk) window.desk.showNotification('🏆 Achievements coming soon!', 'info', 2000);
}

// Close menu when clicking outside
document.addEventListener('click', (e) => {
    if (window.desk?.userMenuVisible) {
        if (!e.target.closest('.user-menu') && !e.target.closest('.user-profile-btn')) {
            document.getElementById('userMenu').style.display = 'none';
            window.desk.userMenuVisible = false;
        }
    }
});